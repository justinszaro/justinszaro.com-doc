---
id: Other Useful Github Commands
tags:
  - git
  - github
---
...
- `git push -d origin branch_name` - Deletes a remote branch
- `git switch -c new_branch` - Creates a new branch and switches to it (different from checkout?)
...
- Warning: This will undo the commits of fellow collaborators.
...
- `git add (-t / -interactive)` - Enter git's interactive mode
...
- `git merge-base main new_feature` - Returns the commit where the topic branch diverges from the main branch
...
- Can also allow you to edit the last few commits
  - This is destructive, so it should only be used in local branches
- `git rebase -i main new_feature` - Rebase in interactive mode
- `git rebase -i HEAD~3` - Picks up the last three commits and begins interactive mode
