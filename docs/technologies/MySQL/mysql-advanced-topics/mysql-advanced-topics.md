...
- Index Use Cases:
  ...
  - Ordered Queries
- Costs of Indexing
  - Each write operations corresponds with one or more write operations in the index.
  - Storage Space
    - Indexes take space!
- When to use indexes
  - Primary Keys
...
- Two types of stored routines:
  ...
  - Procedures:
    - Procedures are called serarately using the call statement. 
...
- If any of the operations fails, the entire group of operations is considered failed an the database returns to its original state.
```
BEGIN TRANSACTION
  INSERT INTO TABLE1
  INSERT INTO TABLE2
...
- It also improves performance when there are numerious write operations.
...
- NEW. will allow you to access the new row that is bring added. 
...
#### Foreign Keys
- Foreign key contraints keeps data consistent across tables. 
- Allows propogation of changes throughout the database. 
- Foreign keys reference another table.
- UPDATE (any update)
- CASCADE (Update or delete)
- RESTRICT (Doesnt allow the operation)
