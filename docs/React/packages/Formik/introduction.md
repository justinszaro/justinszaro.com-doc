...
- To save time, `useFormik()` returns a helper method called `formik.getFieldProps()` to make it simpler to wire up inputs. It returns the exact group of `onChange`, `onBlur`, `value`, and `checked` for a given field.
